...
export function AdminDashboard() {
  const { clients, getClientStats, resetToSeed } = usePortal();
  ...
  return (
    <AdminShell>
      <AdminPage className="space-y-8">
          ...
          action={
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => {
                  if (confirm("Reset all data to seed defaults?")) resetToSeed();
                }}
        ...
        <section>
          <h2 className="mb-4 text-[13px] font-semibold text-foreground">All accounts</h2>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2.5">
                          <p className="text-[15px] font-semibold text-foreground group-hover:text-brand transition-colors">
                            {client.company}
                          </p>
                          <StatusBadge status={client.projectStatus} />
                        </div>
                        <p className="mt-0.5 text-[13px] text-muted-foreground">
                          {client.name} · {formatCurrency(client.monthlyRetainer)}/mo
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6 pl-[60px] sm:pl-0">
                      <div className="hidden gap-6 text-[12px] text-muted-foreground sm:flex">
                        <span>
                          <span className="font-medium text-foreground">{stats.inProgress}</span> active
                        </span>
                        {stats.awaitingClient > 0 && (
                          <span className="text-amber-400">
                            <span className="font-medium">{stats.awaitingClient}</span> blocked
                          </span>
                        )}
                        <span>Updated {formatRelativeTime(client.lastUpdatedAt)}</span>
                      </div>
                      <ArrowRight
                        className={cn(
